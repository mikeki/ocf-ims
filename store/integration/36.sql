-- FROZEN OCF BASELINE — schema version 36.
--
-- This is a point-in-time snapshot of current.sql at the OCF starting schema
-- (v36). TestMigrateSameAsCurrentSchema loads it, applies every migration from
-- v37 onward, and checks the result still matches the live current.sql — i.e.
-- it verifies OCF's *future* migrations stay consistent with current.sql.
--
-- Do NOT edit this file. It is frozen on purpose. The pre-OCF Burning Man
-- migration chain (and its old 06.sql snapshot) was retired here: OCF launches
-- on a fresh database from current.sql and never replays that legacy upgrade
-- path. See docs/plans/40-domain-model.md and CLAUDE.md (Database Migrations).
