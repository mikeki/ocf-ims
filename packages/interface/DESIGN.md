...
- **Screen transitions are the platform's**, never rebuilt in JS — but under
  reduced motion they become a cross-fade (`useScreenAnimation()`, passed to every
  `<Stack>` as `animation`). iOS does that on its own; Android and web do not.
...
- **Reduced motion drops the scale** and keeps opacity, and ships with the
  animation rather than after it. It reaches motion in exactly two places: the
  press scale and the screen transition.
...
- **`StateFade` is not gated on having actually waited.** It fades on every
  mount, so returning to a cached-empty list stacks a 200 ms fade on the
  platform's push. Gating it needs a `waited` prop threaded from each screen's
  `isLoading`; that lands when 3b touches those screens.
- **`Button`'s loading swap is a hard cut.** A 200 ms opacity transition and a
  blur across the label → spinner swap would read better. Polish, not a defect.
