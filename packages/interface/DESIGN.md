...
- Every text-on-background pair that occurs passes **WCAG AA (4.5:1)** in both
  schemes; the lowest is 4.97:1 (`warning` ink on its own tint, light). The
  table is in the 3a.4 PR.
...
- **The `overlay` and `surfaceSunken` roles are defined but not yet used** —
  ...
  retrofitted later.
- **No brand assets yet.** `app.json` still carries Expo's template icon,
  splash and favicon; replacing them is on the 3a.4 checklist.
