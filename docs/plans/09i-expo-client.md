...
| Step | Owner | Output |
|---|---|---|
| D0 — Design system v0 (during 3a) | Claude Design, reviewed by Miguel | Tokens (colour incl. dark mode, type scale, spacing, radii, elevation), primitives (button, field, list row, badge, sheet, tab bar, toolbar), the state/priority colour language (plan 93 states), brand (icon, splash). A `DESIGN.md` recording the direction in words. |
...
**Gate (all must hold):** the tracer runs on all three platforms against
`docker compose -f docker-compose.dev.yml up` with the seeded users; refresh
proven on web (cookie) and native (body) by shortening the access-token TTL in
`.env`; logout clears the cookie on web and SecureStore on native; a private
incident the user may not view is absent from the list and 404s in detail;
`Interface` CI job green including Playwright smoke; `expo export -p web`
output served by the static container in `docker-compose.local.yml`; a plan-09
