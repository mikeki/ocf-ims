...
| D2 — Dispatch (for 3c) → [09x](09x-dispatch-design.md) (the shell, the table, where an incident opens — for 3c.1; the editor, reports, roster and dashboard get their rounds with their slices) | Claude Design, or an in-code picker round per flow | Wide-screen prototypes: split list/detail, dense incident table with filters, the full incident editor, report review, roster, dashboard. Keyboard flow noted per screen. |
