...
| **3b.1 My work** → [09q](09q-field-flows.md) | Builder | "Mine" filter over `ListIncidents` (created by me, attached, or mentioned — client-side; a server filter is a noted follow-up) and `ListReports`; incident/report rows with unread-change markers. | D1 |
| **3b.2 File and append** → [09r](09r-file-and-append.md) | Builder | New incident (summary, priority, types with "Other" → `ProposeIncidentType`, area picker over `ListAreas`, location description/booth, first entry), append entry with `@mention` picker over `ListPersonnel`, `viewer_may_add_journal` gating; optimistic list updates; invalidations per brief. **On-behalf-of moved to 3b.3** — the incident entry write carries no such field; only the report write reads it (09r). **Built 2026-09-12** — the D2 round chose a hybrid: a tap on the Board's docked bar pulls up the filing form, the incident keeps the docked composer. | 3b.1 |
| **3b.3 Reports** → [09t](09t-reports.md) | Architect (3b.3a) → Builder (3b.3b) | File a report, append, link to an incident, `may_edit_summary`/`may_add_journal_entry` gating; "on behalf of" (moved here from 3b.2). **The model decision landed 2026-09-12 (09q Q4: keep both — a report is its own record, and an incident collects reports through a *request*).** 3b.3a adds `RequestReport` (the ask on the involvement row, a `report_requested` notification + push, the grant), 3b.3b the report form, the report composer, the incident's asked / delivered states and the Board's "owes a report" rule. **Both built 2026-09-12** as a stack of two PRs; the phone hand check is owed. | 3b.2 |
| **3b.4 Photos** → [09s](09s-photos.md) | Builder (Architect reviews) | Camera/library via `expo-image-picker`, upload via `src/api/blobs.ts` with progress and retry, preview via headers on native / blob URL on web, `attach_files` permission gating. **Brief written 2026-09-12** — it found the REST upload route behind the Connect writes (no 52f grant path, no privacy gate) and that `Attachment` does not say what the file is; **3b.4a** (server: the gate mirrored, `Attachment.media_type`) went first (merged 2026-09-12); **3b.4b built 2026-09-12** — the blob helper over `XMLHttpRequest`, the picker seam, the composer's chip with a progress bar and Retry, the form's file-then-upload, inline images and a full-width modal; the phone hand check is owed. | 3b.2 |
| **3b.5 Notifications + push** → [09u](09u-alerts-and-push.md) | Builder (Architect owns the token/permission flow) | Alerts tab: list, unread badge, mark one/all read, tap → deep link; push permission prompt on first alert-worthy action (not on launch), token registration/unregistration on sign in/out, foreground handling. **Built 2026-09-12** — the Alerts screen and the header's "Alerts" word with the count (no tab bar exists; 09u Q2), the `src/push/` seam over `expo-notifications`, the prompt from the screen's card, registration on sign-in and unregistration through a `beforeSignOut` hook; the EAS project id (09u Q1) and the phone check are owed. | 3b.0 |
| **3b.6 Live updates** → [09v](09v-live-updates.md) | Architect (client stream) + Builder (wiring) | `src/api/stream.ts` consuming `WatchEvent` via `expo/fetch`; reconnect with backoff and a full refetch after any gap (the templ client's `last_sse_id` check, done with a resume cursor on the stream); pause in background, resume + refetch on foreground; poll fallback when the stream is unavailable. Web: one stream per tab to start; the templ single-tab `navigator.locks` leader + `BroadcastChannel` fan-out is the known optimisation if tab counts hurt. **Built 2026-09-12** — the stream loop (backoff, staleness, gap), a reference-counted hub per event on AppState, pokes → invalidations; the phone check is owed. | 3b.0 |
| **3b.7 Store readiness** → [09w](09w-store-readiness.md) | Mechanic (Builder for assets) | `app.json` (bundle ids, scheme, icons/splash from D0, permissions strings), `eas.json` (development / preview / production), TestFlight + Play internal testing, privacy manifest, `buf breaking` in CI, plan-09 Phase 0 freeze note with the date, Maestro smoke (optional). **Repository half built 2026-09-12** — identifiers (placeholder, 09w Q1), splash, privacy manifest, `eas.json`, `buf breaking` in CI; the accounts, `eas init`, the first builds and the device checks are the maintainer's. | 3b.1–3b.6 |
