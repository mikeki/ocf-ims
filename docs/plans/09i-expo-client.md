...
| **3c.4 Roster** → [09aa](09aa-roster-design.md) | Design (the round) → Architect (criteria) → Builder (Architect reviews) | Per-event people (`ListPersonnel` with `event_id`, `all`), the participation ladder with counts, crews column, profile card gated by role (email/phone admin-only), role change within the server's ceiling, remove from event, add person search-first (enrol or create), invite reporters where `invite_reporters`, My crews for a crew leader. Wide only. **Round brief written 2026-09-14; the round next.** | 3c.1 |
