...
| D1 — Field flows (for 3b); first round → [09q](09q-field-flows.md) ("My work", for 3b.1) | Claude Design, or an in-code picker round per flow | Mobile prototypes: sign in, pick event, my incidents, incident detail, new incident, append entry (+ mention, + photo), reports, notifications, settings. Every empty/error/loading state. |
| D2 — Dispatch (for 3c) → [09x](09x-dispatch-design.md) (the shell, the table, where an incident opens — for 3c.1) → [09y](09y-incident-editor-design.md) (the editor — for 3c.2) → [09z](09z-reports-design.md) (the report pane — for 3c.3) → [09aa](09aa-roster-design.md) (the roster — for 3c.4) → [09ab](09ab-dashboard-design.md) (the dashboard — for 3c.5) | Claude Design, or an in-code picker round per flow | Wide-screen prototypes: split list/detail, dense incident table with filters, the full incident editor, report review, roster, dashboard. Keyboard flow noted per screen. |
...
| Task | Owner | Deliverable | Depends on |
|---|---|---|---|
| **3b.0 Server: live stream + native push** → [09p](09p-stream-and-push.md) | Architect | `WatchEvent(WatchEventRequest{event_id}) returns (stream EventPoke)` — per-subscriber privacy filter (`mayViewIncident`), heartbeat every 25 s, cancellation on client close, `event_access` re-check on each poke, tests through the generated client; `RegisterPushDevice`/`UnregisterPushDevice` + `KIND` migration + `ExpoPushSender` (receipts checked, invalid tokens pruned) wired into the existing notification fan-out; `IMS_EXPO_PUSH_ENABLED`. Findings on connect-go streaming. **Brief written 2026-09-10** — and it found a prerequisite: the interceptor spine is unary-only (`connect.UnaryInterceptorFunc` pass-through), so a streaming RPC would run with no auth, recovery, request id or logging. **3b.0a landed that fix on 2026-09-10** (the five interceptors are real `connect.Interceptor`s now, unary and streaming sharing one body each); it was taken ahead of the 3a gate because it adds no RPC, is provable in Go tests alone, and closes a latent hole in code that already ships. **3b.0b landed the stream on 2026-09-10** — `WatchEvent`, per-subscriber filtering that retires the SSE redaction residual for stream subscribers, proven through the generated client over real HTTP. **3b.0c landed native push on 2026-09-10** — `KIND` on the subscription, `Register`/`UnregisterPushDevice`, `ExpoPushSender` with receipt-driven pruning, all behind `IMS_EXPO_PUSH_ENABLED` (off by default). **All three slices are built; the device and two-client verification is still owed.** | 3a gate (3b.0 built ahead of it) |
| **3b.1 My work** → [09q](09q-field-flows.md) | Builder | "Mine" filter over `ListIncidents` (created by me, attached, or mentioned — client-side; a server filter is a noted follow-up) and `ListReports`; incident/report rows with unread-change markers. **Merged 2026-09-12 (#256)** — the D1 round chose Segmented; the phone check is owed. | D1 |
| **3b.2 File and append** → [09r](09r-file-and-append.md) | Builder | New incident (summary, priority, types with "Other" → `ProposeIncidentType`, area picker over `ListAreas`, location description/booth, first entry), append entry with `@mention` picker over `ListPersonnel`, `viewer_may_add_journal` gating; optimistic list updates; invalidations per brief. **On-behalf-of moved to 3b.3** — the incident entry write carries no such field; only the report write reads it (09r). **Merged 2026-09-12 (#258)** — the D2 round chose a hybrid: a tap on the Board's docked bar pulls up the filing form, the incident keeps the docked composer. | 3b.1 |
| **3b.3 Reports** → [09t](09t-reports.md) | Architect (3b.3a) → Builder (3b.3b) | File a report, append, link to an incident, `may_edit_summary`/`may_add_journal_entry` gating; "on behalf of" (moved here from 3b.2). **The model decision landed 2026-09-12 (09q Q4: keep both — a report is its own record, and an incident collects reports through a *request*).** 3b.3a adds `RequestReport` (the ask on the involvement row, a `report_requested` notification + push, the grant), 3b.3b the report form, the report composer, the incident's asked / delivered states and the Board's "owes a report" rule. **Both merged 2026-09-13 (#261, #262)**; the phone hand check is owed. | 3b.2 |
| **3b.4 Photos** → [09s](09s-photos.md) | Builder (Architect reviews) | Camera/library via `expo-image-picker`, upload via `src/api/blobs.ts` with progress and retry, preview via headers on native / blob URL on web, `attach_files` permission gating. **Brief written 2026-09-12** — it found the REST upload route behind the Connect writes (no 52f grant path, no privacy gate) and that `Attachment` does not say what the file is; **3b.4a** (server: the gate mirrored, `Attachment.media_type`) went first (merged 2026-09-12); **3b.4b merged 2026-09-13 (#263)** — the blob helper over `XMLHttpRequest`, the picker seam, the composer's chip with a progress bar and Retry, the form's file-then-upload, inline images and a full-width modal; the phone hand check is owed. | 3b.2 |
| **3b.5 Notifications + push** → [09u](09u-alerts-and-push.md) | Builder (Architect owns the token/permission flow) | Alerts tab: list, unread badge, mark one/all read, tap → deep link; push permission prompt on first alert-worthy action (not on launch), token registration/unregistration on sign in/out, foreground handling. **Merged 2026-09-13 (#264)** — the Alerts screen and the header's "Alerts" word with the count (no tab bar exists; 09u Q2), the `src/push/` seam over `expo-notifications`, the prompt from the screen's card, registration on sign-in and unregistration through a `beforeSignOut` hook; the EAS project id (09u Q1) and the phone check are owed. | 3b.0 |
| **3b.6 Live updates** → [09v](09v-live-updates.md) | Architect (client stream) + Builder (wiring) | `src/api/stream.ts` consuming `WatchEvent` via `expo/fetch`; reconnect with backoff and a full refetch after any gap (the templ client's `last_sse_id` check, done with a resume cursor on the stream); pause in background, resume + refetch on foreground; poll fallback when the stream is unavailable. Web: one stream per tab to start; the templ single-tab `navigator.locks` leader + `BroadcastChannel` fan-out is the known optimisation if tab counts hurt. **Merged 2026-09-13 (#265)** — the stream loop (backoff, staleness, gap), a reference-counted hub per event on AppState, pokes → invalidations; the phone check is owed. | 3b.0 |
| **3b.7 Store readiness** → [09w](09w-store-readiness.md) | Mechanic (Builder for assets) | `app.json` (bundle ids, scheme, icons/splash from D0, permissions strings), `eas.json` (development / preview / production), TestFlight + Play internal testing, privacy manifest, `buf breaking` in CI, plan-09 Phase 0 freeze note with the date, Maestro smoke (optional). **Repository half merged 2026-09-13 (#266)** — identifiers (placeholder, 09w Q1), splash, privacy manifest, `eas.json`, `buf breaking` in CI; the accounts, `eas init`, the first builds and the device checks are the maintainer's. | 3b.1–3b.6 |
...
**Gate status, 2026-09-13.** Every 3b slice is on master (#256–#266, the last six merged
this morning); the tracer against staging walks file → append → photo → report → live
poke → alerts → sign out on web. What is left is hardware and accounts, none of it code:

| Gate row | State |
|---|---|
| Store-distributed build | ⛔ Maintainer: Apple / Google accounts, `eas init` (→ `extra.eas.projectId`), confirm the placeholder identifiers (09w Q1), first `preview` builds |
| Push arrives on iOS and Android | ⛔ Needs the project id above and `IMS_EXPO_PUSH_ENABLED` on staging; the server side and the client registration are built and unit-tested |
| The stream survives airplane mode and a 10-minute background | ⛔ Device row — the reconnect / refetch logic is unit-tested (09v); the phone run through Caddy is owed |
| Two non-developers complete the flow on both platforms | ⛔ After the builds ship |
| Field numbers frozen and recorded | ⏳ `buf breaking` guards CI since #266; the date is recorded in plan 09 at the first non-developer install |
...
| Task | Owner | Deliverable | Depends on |
|---|---|---|---|
| **3c.0 Dispatch design (D2)** → [09x](09x-dispatch-design.md) | Claude Design → Architect brief | Split list/detail, dense table, filters, editor, keyboard flow, state/priority language. **Brief written 2026-09-13** — taken ahead of the 3b gate on the 3b.0 precedent (the gate is held by accounts and hardware, not code). It found no contract gap; it fixes the URL schema, the keyboard map and the live-row rules 3c.1 inherits, and puts one question to the round: where an incident opens from the table (Split / Drawer / Page) — plus the shell, since E15's sidebar has no code behind it. **Round run and picked 2026-09-13 (#272): Drawer + top bar, the drawer opens the full page; the 3c.1 criteria follow in 09x.** | 3b gate (3c.0 taken ahead of it) |
| **3c.1 Incident table** → [09x](09x-dispatch-design.md) § "3c.1 acceptance criteria" | Builder (fresh Sonnet session; the hub change architect-reviewed) | Columns (number, state, priority, types, area, summary, started, last modified, attached people), sort, search, filters (state/priority/type/area/person, "mine", days), filter state in the URL, live rows patched from the stream, keyboard navigation; the top-bar shell; row → the drawer, Enter again → the full page (the D2 pick). Below 1024 px nothing changes. **Criteria written 2026-09-13; built 2026-09-14 (#274).** | 3c.0 |
| **3c.2 Incident editor** → [09y](09y-incident-editor-design.md) | Design (the round) → Architect (criteria) → Builder (Architect reviews) | Every `IncidentUpdate` field: state, priority, started, types (multi + propose), area, outcome (`ListOutcomes`, propose), summary, location, people (attach/detach, involvement, grant access, `has_event_access`), linked incidents, linked reports (`Int32List` clear-vs-unchanged semantics), private toggle (admin/creator only), entries with mentions (on-behalf-of is read-only here: reports-only on the wire, 09y Q4), strike entry, changes-history/stricken/attached-reports toggles, attachments upload/preview. One field per request; a poke never overwrites an unsaved field. **Merged 2026-09-14 (#278) — the Ledger; the staging hand check is owed.** | 3c.1 |
| **3c.3 Reports** → [09z](09z-reports-design.md) | Design (the round) → Architect (criteria) → Builder (Architect reviews) | Report table (the 3c.1 table with report columns, Unlinked · Linked · All, a Reports item in the shell) + the report pane (summary edit, link / detach, strike, history / stricken toggles, photo on the composer), crew-leader review reads (the server's scoping, shown as answered). **Round brief written 2026-09-14; the round next.** | 3c.2 |
| **3c.4 Roster** → [09aa](09aa-roster-design.md) | Design (the round) → Architect (criteria) → Builder (Architect reviews) | Per-event people (`ListPersonnel` with `event_id`, `all`), the participation ladder with counts, crews column, profile card gated by role (email/phone admin-only), role change within the server's ceiling, remove from event, add person search-first (enrol or create), invite reporters where `invite_reporters`, My crews for a crew leader. Wide only. **Round run 2026-09-15; pick Ladder + drag + hovercard 2026-09-16; criteria in 09aa; two builders next.** | 3c.1 |
| **3c.5 Dashboard** → [09ab](09ab-dashboard-design.md) | Design (the round) → Architect (criteria) → Builder (Architect reviews) | `GetMetrics` — counts by priority/category/type/role/area, by day, open follow-ups, avg time to close; the refresh toolbar and auto-refresh; **no chart library recommended** (bars and columns are Views; 09ab § The chart library). Wide only. **Round brief written 2026-09-15; the round next.** | 3c.1 |
...
3. **Chart library** for the dashboard — 09ab recommends none (bars and columns as
   Views, identical on web and native); the 3c.5 round confirms.
