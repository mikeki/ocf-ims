...
|---|---|---|
| `common/v1/person_ref.proto` | `PersonRef` | `json.Mention` |
| `common/v1/incident_ref.proto` | `IncidentRef` | `json.LinkedIncident` |
...
| `resources/v1/incident.proto` | `Incident`, `IncidentLocation`, `IncidentPerson`, enums `IncidentState`/`IncidentPriority` | `json.Incident` et al. |
| `resources/v1/report.proto` | `Report` | `json.Report` |

### Review changes (PR #203 feedback)

- **`Mention` → `PersonRef`** (still in `common/v1`) — the shared person
  display-ref, named for what it is rather than the journal-specific "mention".
- **`Location` → `IncidentLocation`** — it is incident-specific.
- **`IncidentPerson` embeds `PersonRef`** rather than repeating flat
  `person_id`/`handle`/`name`, so the person reference is defined once.
- **`LinkedIncident` → `IncidentRef`, moved to `common/v1`** — a shared
  cross-resource reference type alongside `PersonRef`. (It has one consumer today,
  so this is slightly ahead of M3's "second consumer" rule, but grouping the
  `*Ref` types is the intent and reuse is likely, e.g. `Report.incident`.)
- **`Attachment.name` → `id`** — the field is a server-generated crypto-random
  storage token, not a user filename, so it is an opaque identifier (and already
  collision-resistant); `id` names it honestly.
- **Minimal license headers on protos** — a one-line
  `// SPDX-License-Identifier: Apache-2.0` replaces the ~15-line block. To keep it
  clean, `prependlicense` now also skips `// @generated by` files (protoc-gen-es
  output) and prunes `node_modules` at any depth (its old `/node_modules/`
  substring check missed the top-level one). The full-header convention on
  hand-written `.go/.templ/.ts` is unchanged.
- Kept as-is after discussion: vendored protovalidate (hermetic), scalar `int32`
  ids (idiomatic for this int-keyed domain).
