...
| [09j-session-contract-cors.md](09j-session-contract-cors.md) | Phase 3, slice 3a.0 — the session contract for a native client (body-carried refresh token, body-wins refresh, `Logout` RPC), report reads marked NSE, dev CORS allow-list (`IMS_CORS_ALLOWED_ORIGINS`) | Built — for review |
